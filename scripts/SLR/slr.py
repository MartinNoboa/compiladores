...
# @last_modified: November 21, 2022
# ------------------------------------
...
def grammarAugmentation(input, nonterm_userdef,initial):
    ...
    newRules.append([newChar,['.', initial]])
    # new format  => [LHS,[.RHS]],
...
def follow(nt):
    global initial, rules, nonterm_userdef, \
...
# @output : cleaned up list
    if (exists(filename)):
        with open(filename) as file:
            ...
            for line in clean:
                temp = line.split("->",1)
                nonterminal.append(temp[0].strip())
...
def findTerminal(r,l):
    ...
    for i in l:
        ...
        if match == False:
            t.append(i)
    terminal = []
    [terminal.append(x) for x in t if x not in terminal]
    return terminal      
...
terminal = findTerminal(nonterminal,right)
print("terminal")
print(terminal)
print("nonterminal")
print(nonterminal)
...
separatedRulesList = grammarAugmentation(input,nonterminal,initial)
# find closure
initial = separatedRulesList[0][0]
I0 = findClosure(0, initial)
...
createParseTable(statesDict, stateMap,terminal,nonterminal)